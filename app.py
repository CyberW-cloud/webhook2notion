		...
		for room in rooms:
			...
			try:
				...
				if "stories_list" not in messages.keys():
					...
					if messages["stories_list"]["stories"][0] != messages_api.get_room_messages(user_id, room["roomId"], {"limit":15})["stories_list"]["stories"][0]:
						i = 1/0 #debug
				else:
					if messages["stories_list"]["stories"][0] != messages_api.get_room_messages(os.environ.get("TeamID"), room["roomId"], {"limit":15})["stories_list"]["stories"][0]:
						i = 1/0
			except Exception as e:
				if "zero" in str(e):
					i = 1/0 #debug
